...
export default function TransactionTrackingView(proping) {
  ...
  const [heightSmp, setHeightSmp] = React.useState(0);
    ...
    try {
      ...
      axios({
        ...
        .then(function (response) {
          ...
          if (response.data.status == 200) {
            ...
            if (response.data.data.debit_note_array.length == 0) {
              ...
              console.log('debitnotearray>>>>>', response.data.data.debit_note_array)
            ...
            // let d = props.Props.debit_note_array.map(item => console.log('item',item))
            // setDebitNoteImageSrc(d)
            // console.log('d', d)
        ...
        .catch(function (error) {
          ...
          alert('ere' + defaultMessages.en.serverNotRespondingMsg);
        });
    } catch (error) {
      console.log(error);
    }
    ...
    return (
      <View style={{
        ...
        {(showDebitNote || setshowDebitNoteMore) ? <TouchableOpacity onPress={props.onPress}
          style={{ width: 'auto',paddingHorizontal:10, borderRadius: 5, justifyContent: 'center', height: 35, alignItems: 'center', backgroundColor: theme.colors.primary }}>
        ...
        <View style={{ flexDirection: 'column' }}>
          ...
          <View style={{ flexDirection: 'row', }}>
            <View style={{ width: '20%', alignItems: 'center' }}>
              ...
              <View style={styles.HeightIndicator(heightDebitNote, showGst ? '#d1d1d1' : theme.colors.primary)} />
            </View>
            <View style={{ width: '83%', justifyContent: 'flex-start', alignItems: 'flex-start' }}>
              ...
              <View style={{ width: '100%', }} onLayout={onLayoutDebitNote}>
                {showDebitNote ?
                  <ButtonLabelDebitNote Title='Upload' show={true} onPress={() => openDocumentPicker('debitnote')} />
                  :
