...
const MainSection = (props) => (
    <View style={{ marginTop: hp(2), paddingHorizontal: wp(5), alignSelf: 'flex-start', backgroundColor: 'transparent' }}>
...
const UpdateScreen = ({ navigation, ref, route }) => {
    ...
    const [filePath, setFilePath] = useState({ assets: [{ uri: data.stamp_image}]});
    const [HeaderfilePath, setHeaderFilePath] = useState({ assets: [{ uri: data.header_image }] });
    // const [FooterfilePath, setFooterFilePath] = useState({});

    const [stampImageError, setStampImageError] = useState(null);
    const [HeaderstampImageError, setHeaderStampImageError] = useState(null);
    // const [FooterstampImageError, setFooterStampImageError] = useState(null);
    ...
    const callUpdate = (id) => {
        ...
        axios({
            url: api_config.BASE_URL + api_config.EDIT_PROFILE_BUYER,
            method: 'POST',
            data: formData,
            headers: {
                Accept: 'application/json',
    ...
    const HeadercaptureImage = async type => {
        let options = {
            mediaType: type,
            maxWidth: 300,
            maxHeight: 550,
        ...
        if (isCameraPermitted && isStoragePermitted) {
            launchCamera(options, response => {
                ...
                setHeaderStampImageError(null);
            });
        }
    };
    const FootercaptureImage = async type => {
        let options = {
            mediaType: type,
            maxWidth: 300,
            maxHeight: 550,
    ...
    const onUploadHeaderStampClicked = () => {
        HeaderactionSheet.current.show();
    };

    const onUploadFooterStampClicked = () => {
        FooteractionSheet.current.show();
    };
                ...
                <ScrollView
                    ...
                    <View pointerEvents={loading ? 'none' : 'auto'}>
                        ...
                        <View style={{
                            ...
                            flexDirection: 'column', paddingHorizontal: wp(4)
                        }}>
                        ...
                        <View style={{ marginTop: 5, marginBottom: 5 }}>
                            ...
                            <View style={styles.container}>
                                {HeaderstampImageError != null ? (
                                    <Text style={styles.error}>{HeaderstampImageError}</Text>
                                ) : null}
                            </View>
                            {/* <View style={{ marginTop: 0, marginBottom: 5, marginLeft: '5%' }}>
                                ...
                                <TouchableOpacity onPress={onUploadFooterStampClicked}>
                            ...
                            <View style={styles.container}>
                                {FooterstampImageError != null ? (
                                    <Text style={styles.error}>{FooterstampImageError}</Text>
                                ) : null}
                            </View> */}
                            <View style={{ marginTop: 0, marginBottom: 5, marginLeft: '5%' }}>
                                <Text
                    ...
                    <View style={styles.container}>
                        ...
                        <ActionSheet
                            ...
                            onPress={index => {
                                ...
                                //alert(index)
                                if (index == 0) {
                                    HeaderchooseFile('photo');
                                } else if (index == 1) {
                                    HeadercaptureImage('photo');
                                }
                                //alert(optionArray[index]);
                            }}
                        />
                        {/* <ActionSheet
                            ref={FooteractionSheet}
                            // Title of the Bottom Sheet
                            title={'Which one do you like ?'}
                            // Options Array to show in bottom sheet
                            options={optionArray}
                            ...
                            onPress={index => {
                                ...
                                //alert(index)
                                if (index == 0) {
                                    FooterchooseFile('photo');
                                } else if (index == 1) {
                                    FootercaptureImage('photo');
                                }
                                //alert(optionArray[index]);
                            }}
                        /> */}
